...
def split_nodes_delimiter(old_nodes, delimiter, text_type):
    ...
    for node in old_nodes:
        if node.find(delimiter)==-1:
            raise Exception(f"delimiter not found in {node.to_html()}")
...
def find_delimiter(text, delimiter, text_type):
    ...
    nodes.append(TextNode(text[start+1:end], text_type))

    if len(text)==end+1:
        return nodes
    else:
        nodes.extend(find_delimiter(text[end+1:], delimiter, text_type))
